...
protocol, detailed in Appendix~\ref{appendix:announce}, that builds on top of the remote implementation
...
For example, rather than putting all objects in a channel in one file,
those files could be subdivided according to their schemas.
This would maintain the simple server architecture but make \texttt{discover} calls more efficient.
...
the ability to grant an application permission only to read objects
