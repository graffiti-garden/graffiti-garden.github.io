...
using a \texttt{for await} loop, rather than waiting for all objects
to be fetched before processing them.

Like \texttt{get}, \texttt{discover} requires a JSON schema.
In addition to providing type safety, it is critical to let developers to
only have to process objects they are interested in. For example,
the channel represented by an actor's URI has many overloaded uses,
like storing a user's profile

Underlying implementations can take advantage of this, too.

In addition to an array of channels, like \texttt{get},
a
