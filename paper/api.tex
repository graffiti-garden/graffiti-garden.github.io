...
\hl{When undefined, any actor can access the object given that the know either its \texttt{url} or one of its \texttt{channels}.}
\item
{\sethlcolor{pink}\hl{%
...
for \texttt{get} the \texttt{session} is optional.   The API responds identically for an object that does not exist,
...
The {\sethlcolor{pink}\hl{\texttt{revision}}} field is {\sethlcolor{pink}\hl{increased}} by the underlying implementation on each modification.
