...
\emph{somewhere} that is willing to host their content accessible by a URL.
}%
...
such as 8kun or Gab}~\cite{8kun,gab}\hl{, then it is possible to refute that content and set up off-ramps for
...
other than the meaning ascribed to them by {\sethlcolor{pink}\hl{developers or users}}.
...
Prior to 2019, Instagram had a Following tab where a userhcould see the replies that people they follow had posted across the application. The Following tab led to a priest outing himself as gay among numerous other examples of context collapse~\cite{instagramfollowingtab}.
}, whereas on X, a user's replies made across the application can all be found in their Replies tab.
The difference in these designs reflects these platform's differing design goals of intimacy versus reach.
...
an application should post the replier's reply to the channel
...
regardless of the applications that other people choose to use.
...
Instagram-like replies will never be displayed in X-like Replies tabs.
...
content simultaneously---a post is ``removed'' in one application but not in another---%
it is not possible for an actor's reply to be simultaneously ``seen'' and ``not seen''
by a potential audience member.
We opt to give posters the ability to express their ``targeted imagined audiences''~\cite{imaginedaudience}
...
\hl{%
Despite the face that the ``Obscure'' property implies that
that there is no global directory of channels,
}%
...
\hl{%
This is an unforunate possibility with any public content.
...
\begin{table}[h]
\small
\caption{Channels Meanings and the Applications they Enable}
