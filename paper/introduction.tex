...
Our goal of preventing lock-in leads almost immediately to the conclusion that a users' social data---their posts, likes, friend lists, etc.---\emph{cannot} be stored in any particular social application, because it becomes too easy for that application to make the data inaccessible to other applications.   Instead, we design the Graffiti ecosystem around a metaphorical \emph{wall} that contains every user's content and is \emph{accessible} by \emph{all} applications (modulo access control) but \emph{controlled} by \emph{none} of them.  In this work we describe the design of our wall and the API that is used by clients to interact with it.   Access to the API is provided through a client-side Javascript library that makes it possible to build a diversity of client apps that need no server support beyond what is provided by the wall. 
...
of the system, while using language from computer security literature,
...
%DK I disagree with the terms primary and secondary here for the same reason as I did in the content sectoin.
...
%DK I don't think this is a fair definition of protocols because what these protocols mainly do is *implement the api*.  And yes they implement it in ways that provide trust but we can't forget the first bit
...
While their protocols include effective 
trust mechanisms like federation and end-to-end-encryption,
...
Finally in we wait until Section~\ref{related-work}
