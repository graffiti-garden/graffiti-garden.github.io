...
Subreddits, Facebook groups, and
...
from being \emph{autonomously} extensible (see requirement~\ref{requirements:autonomous-extensibility}),
...
This is similar to Graffiti's channel-based implementation of microblogging described in Section~\ref{concepts:channel-replies}.
...
schema, unlike channels
which are ``permissionless'' but allow arbitrary
moderation structures to be added on top via
reification.
Channel instantiation also does not allow for the technique to be used
when there is not someone clearly responsible for instantiating the room,
as might be the case for topics, external media, or locations.

Still, Cerulean's existence indicates that 
Matrix might be modified to support the Graffiti API without having
...
The AT Protocol and Nostr both use ``stackable
moderation'' where users can layer ``subscriptions'' to various moderator services.
This affordance is a special case of the more general patterns that are possible from total reification, introduced in Section~\ref{concepts:total-reification}.
