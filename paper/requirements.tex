...
\begin{requirement}
\label{req:easy}
   It should be feasible for most communities to build
...
community-built social applications.
...
are tailored to the communities they serve and
they have distict sense of being \emph{lived-in}
that one-size-fits-all platforms lack.
Most importantly these applications are iterated on with feedback from the community,
giving community members autonomy over their shared space, a sense of ownership and control,
and the ability to make their own future~\cite{threeleggedstool, runyourownsocial, archiveoftheirown}.
...
requiring either signifigant technical expertise or the money to outsource the work.
Unpriveleged communities must resort to other means like creating group chats,
Discord servers, Subreddits, Facebook groups, or Mastodon instances.
While these services host many thriving communities,
users are often faced with rigid and inadequate feature sets,
and (in with centralized services) shifting platform policies.
...
\begin{requirement}[Adverserial Interoperability]
\label{req:adverserial-interop}
...
We use the term ``adverserial interoperability''~\cite{adverserialinterop} because it is may not be in the
...
Users should have \emph{autonomy} over their applications.
...
Either self-supressing to appeal to the "lowest-common deminator" or ignoring the collapse
...
Interoperability between applications as required by \ref{req:adverserial-interop} introduces the possiliby
...
as well affordances to mitigate new sources of inter-application context collapse.
...
just as an ecosystem without interoperability (see requirement~\ref{req:adverserial-interop}) can lock users into a
particular set of applications.

For example, in the ActivityPub standard (the protocol underlying Mastodon),
when user $A$ replies to a post by user $OP$, $A$'s appliction
sends $A$'s reply to $OP$'s server, which then automatically forwards
it to users who wish to view the replies to $OP$'s post.
This setup make it impossible to reply to posts that are hosted on servers
that without automatic reply forwarding;
...
Notably, we also require that \emph{moderation} features are also extensible.
...
and our requirement for interoperability (\ref{req:adverserial-interop}).
